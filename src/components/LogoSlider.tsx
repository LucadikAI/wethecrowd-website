    ...
    <section className="py-16 bg-gray-50 border-y border-gray-100 overflow-hidden">
      ...
      <div className="relative flex overflow-hidden">
        <div className="logo-slider-track flex items-center">
          {/* First set of logos */}
          {logos.map((logo, i) => (
            <div key={`logo-1-${i}`} className="flex justify-center px-10 mx-10 grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-300">
              <img src={logo.src} alt={logo.name} className="h-12 object-contain" referrerPolicy="no-referrer" />
            </div>
          ))}
          {/* Second set of logos for seamless loop */}
          {logos.map((logo, i) => (
            <div key={`logo-2-${i}`} className="flex justify-center px-10 mx-10 grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-300">
              <img src={logo.src} alt={logo.name} className="h-12 object-contain" referrerPolicy="no-referrer" />
            </div>
          ))}
        </div>
